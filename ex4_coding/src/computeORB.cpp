 ...
 * @param [in|out] detected FAST keypoints, please set kp.angle according to the definition of oriented FAST
 */
 ...
 * @param keypoints_1 matched keypoints in the first image
 * @param keypoints_2 matched keypoints in the second image
 * @param R21 the rotation matrix from 2 to 1
 * @param t21 the translation from 2 to 1
 */
 ...
 * @param p3d matched 3D points
 * @param p2d matched 2D points
 * @param R21 Rotation from 2 to 1
 * @param t21 translation from 2 to 1
 */
void poseEstimation3D2D(
 ...
 * @param p3d1 matched 3D points
 * @param p3d2 matched 3D points
 * @param R21 Rotation from 2 to 1
 * @param t21 translation from 2 to 1
 */
void poseEstimation3D3D(
        std::vector<cv::Point3f> p3d1,
        std::vector<cv::Point3f> p3d2,
        Eigen::Matrix3d &R21, Eigen::Vector3d &t21
);
...
int main(int argc, char **argv) {
    ...
    cv::Mat first_depth_image = cv::imread(first_depth_file, -1);    // load grayscale image
    cv::Mat second_depth_image = cv::imread(second_depth_file, -1);  // load grayscale image
    double depth_scale = 5000.0;    // the depth scale of the depth image
    ...
    Eigen::Matrix3d R21_22;
    Eigen::Vector3d t21_22;
    poseEstimation2d2d(kpm1, kpm2, R21_22, t21_22);
    cout << "estimated motion using 2D-2D method" << endl;
    cout << "Rotation from 1 to 2: \n" << R21_22 << endl;
    cout << "translation from 1 to 2: \n" << t21_22.transpose() << endl;

    // 3D-2D transform
    vector<cv::Point3f> p3d;
    vector<cv::Point2f> p2d;
    Eigen::Matrix3d R21_32;
    ...
    cout << "Rotation from 1 to 2: \n" << R21_32 << endl;
    cout << "translation from 1 to 2: \n" << t21_32.transpose() << endl;

    // 3D-3D transform
    vector<cv::Point3f> p3d1;
    vector<cv::Point3f> p3d2;
    Eigen::Matrix3d R21_33;
    ...
    cout << "Rotation from 1 to 2: \n" << R21_33 << endl;
    cout << "translation from 1 to 2: \n" << t21_33.transpose() << endl;

    // you can compare the three estimated results, the 2D-2D's translation may have a scale difference
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    int d_max = 50; // threshold, distance larger than this should be ignored
...
void poseEstimation2d2d(
    ...
    // compute the rotation and translation using keypoints_1 and 2
...
void poseEstimation3D3D(
    ...
    // solve 3D-3D icp problem (given matches)
...
// -------------------------------------------------------------------------------------------------- //
// ORB pattern
// each line is (px, py), (qx, qy)
// you need to rotate it according to the angle before using them
